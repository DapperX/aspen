...
struct treeplus {
  ...
  void map_elms(uintV src, F f) const {
    ...
    auto get_aug_size = [&](Node* a) -> uintV { return Tree::aug_val(a); };
  ...
  template <class P>
  static auto make_count_struct(P p, uintV src) {
    return count_struct<P>(p, src);
  }
  ...
  struct num_nodes_struct {
    ...
    num_nodes_struct() {}
    size_t identity() { return static_cast<size_t>(0); }
    size_t from_entry(const Entry& entry) { return 1; }
    size_t combine(size_t l, size_t r) { return l + r; }
  };
  ...
  struct check_ref_cts_struct {
    ...
    size_t identity() { return static_cast<size_t>(0); }
    size_t from_node(const Tree::node* node) { return node->ref_cnt; }
    size_t combine(size_t l, size_t r) { return l + r; }
  };
  ...
  treeplus(SQ const& S, uintV src, pbbs::flags fl = pbbs::no_flag) {
    ...
    uintV* starts = (S.size() > start_array_size)
                        ? pbbs::new_array_no_init<uintV>(S.size())
                        : starts_backing;
    parallel_for(
        0, S.size(),
        [&](size_t i) {
          if (lists::is_head(S[i]))
            starts[i] = i;
          else
            starts[i] = UINT_V_MAX;
        },
    ...
    auto is_head_seq = pbbs::delayed_seq<bool>(
        S.size(), [&](size_t i) { return lists::is_head(S[i]); });
    ...
    KV* kvs = (head_indices.size() > kv_arr_size)
                  ? pbbs::new_array_no_init<KV>(head_indices.size())
                  : kv_stack;
    ...
    parallel_for(
        ...
        [&](size_t i) {
          ...
          uintV end =
              (i == (head_indices.size() - 1)) ? S.size() : head_indices[i + 1];
    ...
    tree.root = nullptr;
    //      deg = S.size();
    if (S.size() > start_array_size) {
      pbbs::free_array(starts);
    }
    if (head_indices.size() > kv_arr_size) {
      pbbs::free_array(kvs);
    }
  }
};
...
struct treeplus_c {
  ...
  inline maybe<uintV> get_leftmost_key() { return leftmost_key(root); }
  ...
  void print_structure(AT* plus, Node* root) const {
    ...
    if (root) {
      ...
      T.iter_nodes_cond(iter_f);
      T.root = nullptr;
      cout << "root ref-cnt = " << root->ref_cnt << " total deg = " << degree()
           << endl;
    } else {
  ...
  void print_structure(uintV src) const { print_structure(plus, root); }
...
inline Node* copy_tree_node(Node* node, uintV src) {
  if (node) {
    ...
    Tree_GC::increment(node->lc);
    Tree_GC::increment(node->rc);
...
static split_info split(treeplus_c T, const uintV& src,
                        const uintV& split_key) {
  AT* plus = T.plus;
  Node* bst = T.root;
  if (T.empty()) {
    return split_info(T, T, false);
  }

  if (plus) {
    uintV first_key;
    uintV last_key;
    std::tie(first_key, last_key) = lists::first_and_last_keys(plus, src);
    assert(first_key != UINT_V_MAX);
    assert(last_key != UINT_V_MAX);
    if (first_key <= split_key && split_key <= last_key) {  // splits plus
      AT* lt;
      AT* gt;
      bool found_key;
      std::tie(lt, gt, found_key) = lists::split_node(plus, src, split_key);
      Tree_GC::increment(bst);
      return split_info(treeplus_c(lt, nullptr), treeplus_c(gt, bst),
                        found_key);
    } else if (split_key < first_key) {  // left of (plus, bst).
      auto cpy = lists::copy_node(plus);
      Tree_GC::increment(bst);
      return split_info(treeplus_c(nullptr, nullptr), treeplus_c(cpy, bst),
                        false);
  ...
  if (key < split_key) {
    ...
    if (last_key != UINT_V_MAX && split_key <= last_key) {
      ...
      bstpair.first =
          treeplus_c(lists::copy_node(plus),
    ...
    assert(bstpair.first.plus == nullptr);
    bstpair.first = treeplus_c(lists::copy_node(plus), bstpair.first.root);
    bstpair.second =
        treeplus_c(bstpair.second.plus,
...
// later.
static treeplus_c union_bc(AT* b1_plus, treeplus_c b2, uintV src,
                           bool run_seq) {
  ...
  if (to_insert) {
    ...
    auto create_new_key = [&](uintV key, size_t start, size_t end) {
      ...
      if (mk == nullptr) {
        ...
        cout << "Tried finding key = " << key << " start = " << start
             << " end = " << end << " src = " << src << endl;
      }
      assert(mk != nullptr);
      ...
      if (key_arr) {
        ...
        kvs[distinct_keys++] =
            make_pair(key, lists::copy_from_it(it, deg, src));
      }
    };
    ...
    auto new_root =
        Tree::multi_insert_sorted(b2.root, kvs, distinct_keys, replace, true);
...
// Union Invariant: returned value is only visible to the caller.
static treeplus_c union_rec(treeplus_c b1, treeplus_c b2, uintV src,
                            bool run_seq) {
  ...
  assert(bsts.first.degree() + bsts.second.degree() + ((int)bsts.removed) ==
         b1.degree());
  ...
  if (bsts.second.plus && b2.root->rc) {
    ...
    if (m_leftmost.valid) {
      ...
      if (leftmost <=
          last_key) {  // some of bsts.second.plus nghs must go right.
        bool found_key;
  ...
  if (!split_bsts_second_plus) bsts.second.plus = nullptr;
  ...
  if (b1_to_root_arr) {
    if (new_entry.second) {  // join with the split portion or root_arr.
      ...
      new_entry.second =
          lists::union_nodes(new_entry.second, b1_to_root_arr, src);
      lists::deallocate(to_del);
      lists::deallocate(b1_to_root_arr);
    } else if (!split_root_arr &&
               root_arr) {  // root_arr was not split, join with it.
  ...
  auto P = utils::fork<treeplus_c>(
      !run_seq && utils::do_parallel(n1, n2),
...
static treeplus uniont(const treeplus& b1, const treeplus& b2, uintV src,
                       bool run_seq = false) {
  ...
  auto ret = union_rec(treeplus_c(b1.plus, b1.root),
                       treeplus_c(b2.plus, b2.root), src, run_seq);
  ...
  uintV* ver_edges;
  uintV ver_deg;
  std::tie(ver_edges, ver_deg) =
      uncompressed_lists::union_2(b1_edges, b1.degree(), b2_edges, b2.degree());
  assert(ver_deg = tp.degree());
  pbbs::free_array(b1_edges);
  pbbs::free_array(b2_edges);
  pbbs::free_array(tp_edges);
...
static treeplus_c join_plus_and_other(treeplus_c left, treeplus_c right,
                                      uintV src) {
...
static treeplus_c join_other_and_plus(treeplus_c b1, treeplus_c b2, uintV src) {
  auto mr = b1.rightmost(b1.root);
  assert(mr.valid);
  uintV right_key = mr.value.first;
  auto right_arr = mr.value.second;
...
inline Node* insert_rightmost(Node* root, AT* to_insert, uintV src) {
  auto mr = treeplus_c::rightmost(root);
  assert(mr.valid);
  uintV right_key = mr.value.first;
  auto right_arr = mr.value.second;
...
static treeplus_c join2(treeplus_c b1, treeplus_c b2, uintV src) {
  ...
  if (b1.has_plus() && !b1.has_tree()) return join_plus_and_other(b1, b2, src);
  if (b2.has_plus() && !b2.has_tree()) return join_other_and_plus(b1, b2, src);
  ...
  auto new_root =
      join2_i(new_b1_root, b2.root, /*extra_b1=*/true, /*extra_b2=*/true);
  if (copied_b1) {
...
static treeplus_c difference_bc_1(treeplus_c b1, treeplus_c b2, uintV src,
                                  bool run_seq) {
  if (b2.has_plus() && !b2.has_tree()) {
  ...
  if (to_insert) {
    ...
    auto new_root = Tree::multi_insert_sorted(
        b2.root, kvs, distinct_keys, replace, true);  // removed run_seq
...
static treeplus_c difference_bc_2(treeplus_c b1, treeplus_c b2, uintV src,
                                  bool run_seq) {
  assert(!b2.has_tree());
  auto filter_f = [&](uintV ngh) { return !b1.contains(src, ngh); };
  return treeplus_c(lists::filter(b2.plus, src, filter_f), nullptr);
}

// Returns b2 / b1
// Assumes that b1 and b2 are owned by the caller.
static treeplus_c difference_rec(treeplus_c b1, treeplus_c b2, uintV src,
                                 bool run_seq) {
  if (b1.empty()) return b2.copy_treeplus(src);
  ...
  if (b1_to_root_arr) {
    if (new_entry.second) {  // difference with the split portion or root_arr.
      auto to_del = new_entry.second;
      new_entry.second =
          lists::difference(b1_to_root_arr, new_entry.second, src);
      lists::deallocate(to_del);
      lists::deallocate(b1_to_root_arr);
    } else if (!split_root_arr &&
               root_arr) {  // root_arr was not split, difference.
  ...
  if (P.second.plus) {
    ...
    lists::deallocate(old);
    lists::deallocate(P.second.plus);
  }
...
// Purely functional version
// Returns b2 / b1
static treeplus difference(const treeplus& b1, const treeplus& b2, uintV src,
                           bool run_seq = false) {
//    cout << "Difference: " << b1.degree() << " " << b2.degree() << " src = "
//    << src << endl;
#ifdef CHECK_CORRECTNESS
  b1.check_edge_ct(src);
  b2.check_edge_ct(src);
  ...
  auto ret = difference_rec(treeplus_c(b1.plus, b1.root),
                            treeplus_c(b2.plus, b2.root), src, run_seq);
  auto tp = treeplus(ret.plus, ret.root);
...
//    tp.print_structure(src);
  ...
  uintV* ver_edges;
  uintV ver_deg;
  std::tie(ver_edges, ver_deg) = uncompressed_lists::difference_2(
      b1_edges, b1.degree(), b2_edges, b2.degree());
  //    cout << "ver_deg = " << ver_deg << endl;
  if (ver_deg != tp.degree()) {
    cout << "src = " << src << " ver_deg = " << ver_deg
         << " to = " << tp.degree() << endl;
    cout << "b1.size = " << b1.degree() << " b2.size = " << b2.degree()
         << " src = " << src << endl;
...
static size_t intersect_rec(treeplus_c b1, uintV s1, treeplus_c b2, uintV s2) {
  ...
  if (b1.has_plus() && !b1.has_tree()) return intersect_bc(b1, s1, b2, s2);
  if (b2.has_plus() && !b2.has_tree()) return intersect_bc(b2, s2, b1, s1);
...
static size_t intersect(treeplus& b1, uintV s1, treeplus& b2, uintV s2) {
  size_t check_size = intersect_rec(treeplus_c(b1.plus, b1.root), s1,
                                    treeplus_c(b2.plus, b2.root), s2);

#ifdef CHECK_CORRECTNESS
  auto b1_edges = b1.get_edges(s1);
  auto b2_edges = b2.get_edges(s2);
  size_t ins_size = uncompressed_lists::intersect(b1_edges, b1.degree(),
                                                  b2_edges, b2.degree());
  cout << "intersect = " << check_size << " correct size = " << ins_size
       << endl;

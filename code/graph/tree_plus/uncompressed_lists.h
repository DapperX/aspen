...
template <class F>
inline void map_array(uintV const* node, const uintV& src, size_t offset,
                      const F& f) {
  if (node) {
    size_t deg = node[0];
    uintV const* edges = node + 1;
...
template <class SQ>
uintV* generate_tree_node(uintV key, uintV src, SQ& S, size_t start,
                          size_t end) {
...
template <class SQ>
uintV* generate_plus(SQ& S, size_t first_head_ind, uintV src) {
...
inline tuple<uintV, uintV> first_and_last_keys(uintV const* node,
                                               const uintV& src) {
  ...
  if (node) {
    ...
    uintV const* edges = node + 1;
    first_key = edges[0];
    last_key = edges[deg - 1];
  }
  return make_tuple(first_key, last_key);
}

tuple<uintV*, uintV*, bool> split_node(uintV const* node, uintV src,
                                       uintV split_key) {
  uintV* left = nullptr;
  uintV* right = nullptr;
  bool found = false;
...
uintV* copy_node(uintV const* node) {
  if (node) {
    return copy_arr(node + 1, node[0],
                    0);  // TODO get rid of src where unnecessary
  }
  return nullptr;
}

uintV* union_it_and_node(read_iter& it, size_t l_size, uintV const* r_node,
                         uintV src) {
  assert(r_node);
...
tuple<uintV*, uintV> union_2(uintV const* l, size_t l_size, uintV const* r,
                             size_t r_size) {
  assert(l);
  assert(r);
...
// r / l
tuple<uintV*, uintV> difference_2(uintV const* l, size_t l_size, uintV const* r,
                                  size_t r_size) {
  assert(l);
  assert(r);
  uintV* ret = pbbs::new_array_no_init<uintV>(l_size + r_size);
...
uintV* union_arrs(uintV const* l, size_t l_size, uintV const* r, size_t r_size,
                  uintV src) {
  assert(l);
  assert(r);
  auto write_iter = uncompressed_iter::write_iter(src, l_size + r_size);
